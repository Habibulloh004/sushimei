...
export default function AddressMapPicker({ latitude, longitude, onChange }: AddressMapPickerProps) {
  ...
  return (
    <div className="relative h-full min-h-64 w-full overflow-hidden rounded-xl border border-stone-200">
      <div ref={containerRef} className="h-full w-full" />

      <button
        type="button"
        onClick={handleLocateMe}
        className="absolute right-3 top-3 z-[400] inline-flex items-center gap-2 rounded-xl border border-stone-200 bg-white/95 px-3 py-2 text-[11px] font-bold uppercase tracking-widest text-stone-700 shadow-md backdrop-blur hover:bg-white"
        disabled={locating}
      >
      ...
      {geocoding && (
        <div className="absolute bottom-3 left-3 z-[400] inline-flex items-center gap-2 rounded-xl bg-white/95 px-3 py-2 text-[11px] font-semibold text-stone-600 shadow-md backdrop-blur">
      ...
      {latitude == null || longitude == null ? (
        <div className="pointer-events-none absolute inset-x-0 bottom-3 z-[400] mx-auto w-fit rounded-xl bg-stone-950/80 px-3 py-1.5 text-[11px] font-semibold text-white shadow-lg">
          Xaritadan manzilni bosing
        </div>
      ) : null}
    </div>
  );
